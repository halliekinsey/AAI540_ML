...
def get_headers():
    """Build the headers with your token for authentication."""
...
def load_chat_history(chat_id):
    ...
    plus the raw JSON so we can see usernames if needed.
    """
    if not chat_id:
        return [], "", None
...
def fetch_user_sessions(username):
    """Filter chat sessions to those that contain messages by the given username."""
...
def load_selected_chat_from_dropdown(chat_id):
    """Load selected chat from the dropdown."""
    if not chat_id:
        return [], ""
...
def new_chat():
    """
    Start a new empty conversation with an empty chat_id.
    We'll create the chat_id on the first user message if needed.
    """
    return [], ""

def generate_chat_id_from_question(question: str) -> str:
    """
    Generate a short chat ID from the user's question if no custom ID provided.
    - Lowercase, remove special chars, replace spaces, truncate to 50
    - Fallback to short uuid if empty
    """
...
def generate_chat_response(
    ...
    - If chat_id is empty, derive from user message
    - Append user's message
    - Make POST request to /v1/chat/completions
    - Return updated chat history and possibly updated chat_id
    """
...
def refresh_sessions(username):
    """Refresh sessions for the given user."""
...
def upload_pdf(pdf_path, username, chat_id):
    """
    Immediately upload and index the PDF.
    If successful => enable PDF usage, disable tax optimize
    """
    if not pdf_path:
...
def on_pdf_change(pdf_path, username, chat_id, pdf_loaded_state):
    """
    - Upload PDF
    - If successful, set pdf_loaded_state=True, use_pdf_context=interactive=True, tax_optimize=interactive=False
    - Otherwise do not override states
    """
...
def on_tax_optimize_change(tax_value, pdf_status, use_pdf_context):
    """
    If turning on tax optimize => forcibly disable PDF usage.
    If turning off => re-enable PDF usage if PDF is loaded
    """
    if tax_value:
        return (
            "",
...
def on_pdf_loaded_change(pdf_loaded, current_use_pdf_value):
    """
    If no PDF is loaded, disallow toggling PDF usage. 
    Show a tooltip if user hovers: "Select and upload a PDF first"
    If PDF loaded => keep it interactive
    """
    if pdf_loaded:
        return gr.update(
            interactive=True,
...
def send_feedback(feedback_value, chat_history, username, chat_id, use_pdf_context, tax_optimize):
    ...
    feedback_value is either 'UP' or 'DOWN'.
    chat_history is the entire (role, content) list.
